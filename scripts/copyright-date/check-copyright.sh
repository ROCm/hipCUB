...
if `git config --get --local hooks.updateCopyright`; then
...
if [ -n ${diff_hash} ]; then
    printf "Using the diff: ${diff_hash}\n"
fi
...
if ! $update; then
    printf \
"\nRun '\033[33mScripts/Copyright/check_year.sh -u\033[0m' to update the copyright statement(s). See '-h' for more info.
